...
def play(word):
''' 
не совсем понятен смысл списка guess, поэтому все непраильные буквы сохраняются в список guess_wrong
правильные буквы покажет функция print_game()
'''
    attempt = 6
    guess_wrong = []
    letters = []
    ...
    while attempt > 0:
        ...
        if None not in letters:
            print('> You win!')
            break
        user_letter = get_letter(attempt, guess_wrong)
